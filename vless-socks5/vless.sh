...
EOF
    ...
    # 新增：将节点信息以json格式保存至/opt目录下
    OPT_EXPORT_DIR="/opt"
    OPT_EXPORT_FILE="${OPT_EXPORT_DIR}/xray_node_info.json"
    mkdir -p "$OPT_EXPORT_DIR"
    echo "$CONFIG_JSON" > "$OPT_EXPORT_FILE"
    echo -e "${GREEN}${ICON_SUCCESS} 节点信息已保存为JSON文件：${YELLOW}${OPT_EXPORT_FILE}${NC}"
...
show_final_result() {
    ...
    echo -e "  ${CYAN}节点信息JSON：${YELLOW}/opt/xray_node_info.json${NC}\n"
