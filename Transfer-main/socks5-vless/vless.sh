...
# ========== SOCKS5中转配置 ==========
SOCKS5_CONFIG_FILE="/opt/socks5_config.json"
SOCKS5_SERVER_IP=""
SOCKS5_SERVER_PORT=""
SOCKS5_USERNAME=""
SOCKS5_PASSWORD=""
...
# ========== SOCKS5配置文件读取函数 ==========
read_socks5_config() {
    echo -e "${CYAN}${BOLD}${ICON_CONFIG} 检查SOCKS5配置文件...${NC}"

    if [[ -f "$SOCKS5_CONFIG_FILE" ]]; then
        echo -e "${GREEN}${ICON_SUCCESS} 发现SOCKS5配置文件，启用中转模式${NC}"

        # 检查jq是否可用
        if ! command -v jq &>/dev/null; then
            echo -e "${YELLOW}${ICON_WARNING} jq未安装，正在安装...${NC}"
            $PKG_INSTALL jq >/dev/null 2>&1
        fi

        # 解析SOCKS5配置
        if SOCKS5_SERVER_IP=$(jq -r '.socks5_server.server_ip // empty' "$SOCKS5_CONFIG_FILE" 2>/dev/null) && [[ -n "$SOCKS5_SERVER_IP" ]]; then
            SOCKS5_SERVER_PORT=$(jq -r '.socks5_server.server_port // empty' "$SOCKS5_CONFIG_FILE")
            SOCKS5_USERNAME=$(jq -r '.socks5_server.username // empty' "$SOCKS5_CONFIG_FILE")
            SOCKS5_PASSWORD=$(jq -r '.socks5_server.password // empty' "$SOCKS5_CONFIG_FILE")

            if [[ -n "$SOCKS5_SERVER_IP" && -n "$SOCKS5_SERVER_PORT" && -n "$SOCKS5_USERNAME" && -n "$SOCKS5_PASSWORD" ]]; then
                RELAY_MODE=true
                echo -e "${GREEN}${ICON_SUCCESS} SOCKS5配置读取成功：${NC}"
                echo -e "  ${CYAN}服务器：${YELLOW}${SOCKS5_SERVER_IP}:${SOCKS5_SERVER_PORT}${NC}"
                echo -e "  ${CYAN}用户名：${YELLOW}${SOCKS5_USERNAME}${NC}"
                echo -e "  ${CYAN}模式：${YELLOW}中转模式${NC}"
            else
                echo -e "${YELLOW}${ICON_WARNING} SOCKS5配置不完整，使用直连模式${NC}"
            fi
        else
            echo -e "${YELLOW}${ICON_WARNING} SOCKS5配置解析失败，使用直连模式${NC}"
        fi
    else
        echo -e "${BLUE}${ICON_INFO} 未发现SOCKS5配置文件，使用直连模式${NC}"
    fi
    echo ""
}
...
main_install() {
    ...
    echo -e "${GREEN}${ICON_SUCCESS} 已开放端口：SSH(22,11086), Xray(${PORT})${NC}\n"
    ...
    if [[ "$RELAY_MODE" == true ]]; then
        echo -e "${GREEN}${ICON_INFO} 生成中转模式配置${NC}"
        cat > /etc/xray/config.json << EOF
{
  ...
  "routing": {
    ...
    "rules": [{
      ...
      "type": "field",
      "network": "tcp,udp",
      "outboundTag": "socks5-relay"
    }]
  }
}
EOF
    else
        echo -e "${BLUE}${ICON_INFO} 生成直连模式配置${NC}"
        cat > /etc/xray/config.json << EOF
{
  "log": {
    "loglevel": "warning",
...
show_final_result() {
    ...
    if [[ "$RELAY_MODE" == true ]]; then
        echo -e "  ${CYAN}工作模式：${YELLOW}中转模式${NC}"
        echo -e "  ${CYAN}上游SOCKS5：${YELLOW}${SOCKS5_SERVER_IP}:${SOCKS5_SERVER_PORT}${NC}"
    else
        echo -e "  ${CYAN}工作模式：${YELLOW}直连模式${NC}"
    fi
    echo ""
