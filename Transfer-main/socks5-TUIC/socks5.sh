...
download_transfer_bin() {
    ...
    if curl -L "$TRANSFER_URL" -o "$TRANSFER_BIN" >/dev/null 2>&1; then
        ...
        echo -e "${RED}${ICON_ERROR} transfer 二进制文件下载失败${NC}"
        return 1
    fi
}
...
main() {
    ...
    configure_dante
    start_dante_service
    save_config_to_json  # 修改为保存JSON配置

    # 下载transfer工具
    download_transfer_bin

    # 生成配置JSON并上传
    if [[ -f "/opt/socks5_config.json" ]]; then
        # 读取现有配置
        CONFIG_JSON=$(cat "/opt/socks5_config.json")

        # 保存导出配置到文件
        CONFIG_FILE="/opt/socks5_config_export.json"
